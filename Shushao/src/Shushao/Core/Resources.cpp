...
namespace Shushao {
    ...
    {
        auto data = Shushao::Resources::GetEmbeddedData(IDRES, library, RT_RCDATA);
    ...
    void Resources::Clear()
    {
        for (auto& asset : Assets) {
            if (asset.second != nullptr) {
                //DEBUG_CORE_INFO("Cancello {0}", asset.second->name + " (" + Utility::classtitle(typeid(*asset.second).name()) + ")");
                delete (asset.second);
    ...
    void Resources::toString()
    {
        ...
        for (auto const& asset : Assets) {
            //std::cout << "  - (" << Utility::classtitle(typeid(*asset.second).name()) << ") " << asset.first << std::endl;
        }
